    ...
    >
        <CRUDForm @submit="handleSubmit($event)">
            <CRUDFormField
                ...
                :initial-value="course?.name"
                class="w-full"
            />
            <Button
                v-if="!course?.id"
                v-tooltip.bottom="{
                    value: 'Neuen Kurs mit angegebenem Namen erstellen.',
                    showDelay: 500,
                    hideDelay: 250,
                }"
                label="Speichern"
            ...
            >
                <Button
                    v-tooltip.bottom="{
                        value: 'Änderungen am Kursnamen speichern.',
                        showDelay: 500,
                        hideDelay: 250,
                    }"
                    label="Speichern"
                ...
                <Button
                    v-tooltip.bottom="{
                        value: 'Kurs permanent löschen. Entfernt diesen Kurs aus allen Fragen.',
                        showDelay: 500,
                        hideDelay: 250,
                    }"
                    label="Löschen"
...
const course = ref<Database["public"]["Tables"]["course"]["Row"] | null>(null);
...
const updateData = async (event: FormSubmitEvent) => {
    ...
    const { error } = await supabase
        ...
        .eq("id", course.value?.id as never);
    if (error) {
        simpleToast.error(error.message);
        return;
    }
